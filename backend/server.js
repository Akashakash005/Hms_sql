...
app.post("/regular-appointment/:id", (req, res) => {
  const { firstname, lastname, doctorname, city, date, status } = req.body;
  ...
  db.query(
    ...
    (err, result) => {
      ...
      const insertQuery = `
  INSERT INTO regular_appointment (appointment_no,id, firstname, lastname, doctorname ,city , date , status)
  VALUES (?, ?, ?, ?, ?, ?, ?,?)
`;
      db.query(
        insertQuery,
        [newNo, patientId, firstname, lastname, doctorname, city, date, status],
        (err) => {
          if (err) {
